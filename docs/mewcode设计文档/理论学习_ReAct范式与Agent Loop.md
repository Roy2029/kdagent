...
> 来源：飞书文档（带权限）  
> 抓取日期：2026-08-04  
> 原链接：https://icnaxnmh86kx.feishu.cn/wiki/IlSpwV6rsiKIfOkv2TnczhESnkd

本文讨论了 MewCode 从工具辅助聊天升级为自主编程助手的核心模块——引擎层 Agent 循环（ReAct Loop）的设计逻辑、实现方案与关键特性。关键要点包括：

1.ReAct 范式核心逻辑：即「推理（Think）→ 行动（Act）→ 观察结果（Observe）」交替循环，是 2022 年 Shunyu Yao 等人提出的范式，相比纯推理的 Chain-of-Thought、纯行动的 Act-only 更适配编码场景，Claude API 原生支持该模式。

2.Agent Loop 核心实现：本质是一个 while 循环，持续调用 LLM 并拼接消息上下文，无工具调用需求时终止，是 Coding Agent 的核心机制。

3.Agent Loop 四类必配停止条件：分别为模型主动判定任务完成、设置 50 次迭代上限作为安全网、支持用户 Esc 主动取消、异常状态检测，避免循环失控产生高额 API 费用。

4.事件流解耦设计：采用 AgentEvent 异步流输出模型文本、工具调用/结果等事件，实现 Agent 与 UI 完全解耦，双方可独立替换、测试。

5.工具调用分批逻辑：按工具是否支持并发安全做批次划分，安全工具并发执行、非安全工具串行执行，提升执行效率同时避免副作用冲突。

6.Plan Mode 实现逻辑：通过 Prompt 约束模型仅执行探索类操作、将方案写入指定计划文件，权限系统作为兜底，无需硬禁用工具即可实现规划模式。
...
<!-- 图片: Chain-of-Thought Act-only 与 ReAct 对比 -->
...
| Java | Thread.interrupt / Future.cancel | if (Thread.currentThread().isInterrupted()) return; |
...
<!-- 图片: Esc 中断与 Ctrl+C 退出 -->

第四种：异常状态检测。 如果模型请求调用的工具不存在，比如工具名拼错了，或者那个工具被禁用了，返回一个错误结果让模型自己调整。
...
Agent Loop 的每一轮其实可以用一个非常简单的状态机来理解。模型每次响应之后，只有两种可能：继续循环，或者终止循环。
...
所以 Plan Mode 的实现核心是通过 Prompt 指令约束模型行为。系统注入一段强指令，告诉模型当前是规划模式：
