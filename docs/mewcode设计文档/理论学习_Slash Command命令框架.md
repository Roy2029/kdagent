...
> 来源：飞书文档（带权限）  
> 抓取日期：2026-08-04  
> 原链接：https://icnaxnmh86kx.feishu.cn/wiki/MIDjwkk8HiEZDxkl7PWcsabmn2e

本文讨论了 MewCode 的 Slash Command 命令框架的设计背景、架构逻辑、功能特性及内置命令，用于解决无需 AI 参与的操作走 Agent 引擎消耗 Token、响应慢的问题。关键要点包括：

1. 框架核心定位：该框架是交互层命令框架，所有以 / 开头的输入会被拦截，绕过 Agent Loop 本地处理，实现毫秒级响应、节省 Token，避免清屏、查 Token 等无需 AI 参与的操作被 LLM 无效处理。
2. 框架核心能力：需解决注册、解析、执行三大问题，采用声明式注册统一管理命令信息，解析器按首个空格拆分命令名与参数，执行环节通过统一的 CommandContext 为命令提供所需资源，隔离 UI 实现细节。
3. 命令分类规则：共分三类，local 类不走 Agent Loop 本地执行、local-ui 类本地执行同时修改 UI 状态、prompt 类将预设 prompt 转发给 Agent 处理，会消耗 Token。
4. 用户体验优化设计：支持命令别名、参数提示、Tab 补全功能，别名冲突在启动阶段检测抛出，用户输入无参数的需参数命令时自动提示用法，输入前缀按匹配规则补全命令。
5. 内置命令配置：MewCode v1 内置 9 个命令，含 6 个 local 类命令（/help、/compact 等）、2 个 local-ui 类命令（/clear、/plan）、1 个 prompt 类命令 /review。
6. 框架局限与后续规划：仅支持预定义硬编码操作，无法由用户自定义命令，外部命令扩展、AI 能力结合的命令功能将在下一章的 Skill 系统中实现。
...
<!-- 图片: Slash Command 快车道与 Agent Loop -->
...
<!-- 图片: Web Router 与 Command Registry -->
...
```
[PLAN] tokens: 45,230/200k | /plan 退出 Plan 模式
```
...
理论讲够了，来看看 MewCode v1 到底内置了哪些命令。一共 9 个，按前面说的三种类型来分组介绍。

<!-- 图片: MewCode 命令全家福 -->

### 本地命令

先看六个 local 类命令，不走 Agent Loop，handler 执行完直接返回结果。
...
接下来是两个 local-ui 类命令，它们除了本地执行，还会改变 UI 的行为模式。
...
<!-- 图片: Slash Command 到 Skill 系统的边界 -->
