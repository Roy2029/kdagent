...
> 来源：飞书文档（带权限）  
> 抓取日期：2026-08-04  
> 原链接：https://icnaxnmh86kx.feishu.cn/wiki/ZHysw7gyGi0Rm0k7ElkciqjjnRg

本文讨论了 MewCode 为解决单 Agent 上下文污染问题，设计实现 SubAgent 子任务分发机制的相关架构、实现逻辑与使用规则。关键要点包括：

1. 核心问题与解决思路：单 Agent 处理多任务时会出现上下文污染，导致 Token 消耗飙升、响应质量下降，解决方案是将 Agent 包装为工具，通过主从分发模式创建独立上下文的子 Agent 处理子任务。
2. 两种 SubAgent 创建模式：定义式用于固定角色任务，预定义能力边界、工具集，走前台同步执行；Fork 式用于和当前任务强相关的临时任务，继承父 Agent 完整对话历史，强制后台异步执行，可命中 prompt cache 降低成本。
3. 上下文隔离规则：两类子 Agent 均隔离文件缓存、权限追踪，共享 LLM 客户端、Hook 引擎与文件系统；定义式子 Agent 为空白对话，Fork 式子 Agent 继承父 Agent 对话历史。
4. 工具过滤四层防线：依次为全局禁止列表、自定义 Agent 额外限制、后台 Agent 白名单、Agent 自定义的白/黑名单，避免子 Agent 无限嵌套、资源失控。
5. 后台运行触发方式：共 4 种触发路径，分别为调用时指定 run_in_background、前台任务超 120 秒自动切换、用户按 ESC 手动切换、Fork 模式无条件后台运行，后台任务仅能使用基础工具，不可再创建子 Agent。
6. 内置 4 类预定义 Agent：分别为只读的 Explore 代码探索 Agent、Plan 规划 Agent、通用能力的 general-purpose Agent、需手动开启的 Verification 验证 Agent，覆盖常见开发场景。
...
<!-- 图片: 交互式主 Agent 与 RunToCompletion -->
...
工具过滤因创建模式而异。全局禁止列表、自定义 Agent 限制、后台 Agent 白名单、Agent 定义的 tools/disallowedTools，四层覆盖不同风险。Fork 子 Agent 直接继承父 Agent 的全部工具不做过滤，防递归靠运行时拦截；定义式子 Agent 则经过这四层层层过滤。
