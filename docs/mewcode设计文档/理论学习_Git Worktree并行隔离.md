...
> 来源：飞书文档（带权限）  
> 抓取日期：2026-08-04  
> 原链接：https://icnaxnmh86kx.feishu.cn/wiki/RVcTwYYiGiVNZ7kcxaccub4incc

本文讨论了 MewCode 为解决并行 Agent 的文件系统冲突问题，基于 Git Worktree 实现安全层文件系统隔离的完整方案。关键要点包括：

1. 问题背景：此前 SubAgent 已实现消息、权限、缓存隔离，但后台子 Agent、Agent Team 成员与主 Agent 并行运行时共享同一文件系统，会出现文件覆盖、内容错乱等冲突，Git 分支仅能提供时间维度隔离，无法满足同时多独立工作区的空间隔离需求。
2. Git Worktree 核心特性：该功能自 Git 2.5 引入，支持同一仓库创建多个独立工作目录，各目录对应不同分支互不干扰，且共享同一 .git 仓库、版本历史统一，完美适配并行 Agent 的文件隔离需求。
3. Worktree 核心管理逻辑：MewCode 设计 WorktreeManager 管理其全生命周期，创建前需校验名称防止路径遍历攻击，支持 3ms 级别的快速恢复优化，创建后需完成复制本地配置、配置 Git Hooks、软链接大依赖目录、复制指定忽略文件四项初始化操作。
4. Worktree 进出与清理规则：进入时不修改进程全局工作目录，通过会话状态让工具显式读取路径避免并发冲突；退出时设置变更保护防止误删未提交修改，无变更的临时 Worktree 自动清理，后台定期扫描孤儿临时 Worktree 且保留有未提交修改/未推送 commit 的目录。
5. 与 SubAgent 的配合机制：将 Agent 配置的 isolation 字段设为 worktree 即可自动绑定，子 Agent 启动时自动创建对应 Worktree、注入上下文通知，完成后自动清理并将保留的 Worktree 路径、分支信息返回给主 Agent 供 review。
6. 方案价值：Git Worktree 补上了 SubAgent 隔离体系中文件系统维度的最后一块拼图，结合此前的逻辑层隔离，为子 Agent 提供了完整独立的工作环境，为后续多子 Agent 并行工作提供基础。
...
前台子 Agent 不需要担心这个问题。主 Agent 会等它完成后才继续工作，同一时刻只有一个 Agent 在操作文件，不会冲突。但后台子 Agent 和 Agent Team 的队员（下一章会讲到）就不一样了。它们和主 Agent 或其他队员并行运行，共享同一个文件系统。
...
后续 Agent 调用 Bash、Read、Write 这些工具时，工具自己从 currentSession.WorktreePath 取出路径，作为本次调用的 cwd 传给子进程或文件操作。进程 cwd 还在 originalCwd，每次工具调用是显式声明"在 Worktree 里跑"的。
...
这就是退出时的变更保护。如果 Worktree 里有未提交的文件修改或新增的 commit，直接删除意味着丢失工作成果。LLM 可能不理解这些修改的价值，一个错误的 remove 调用就能把子 Agent 几个小时的工作彻底抹掉。所以 discardChanges 参数要求调用方显式确认丢弃。

<!-- 图片: 退出 Worktree 的变更保护 -->
